...
class LinearRegression:
    ...
    def y_hat(self, X, w):
        #Для получения У предикт, W * X и складываем все значения
        return np.dot(w.T, X)

    def cost(self, yhat, y):
        #Функция потерь в общем виде. Взяли MSE
        C = 1 / self.m * np.sum(np.power(yhat - y, 2))

        return C

    def gradient_descent(self, w, X, y, yhat):
        #Градиентный Спуск. Берем производную от функции Потерь и умножаем на Фичи которые были заданы изначально
